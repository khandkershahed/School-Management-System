        ...
        return this.loader.file.then(
            (file) =>
                new Promise((resolve, reject) => {
                    ...
                    axios
                        .post(imageUploadRoute, formData, {
                            headers: {
                                "Content-Type": "multipart/form-data",
                            },
                        })
...
const ckeditorEl = document.querySelector("#ckeditor");
if (ckeditorEl) {
    ClassicEditor.create(ckeditorEl, {
        language: locale,
        extraPlugins: [CustomUploadAdapterPlugin],
    })
        .then((editor) => {
...
/** ---------------- Rotate icon on search focus (SAFE) ---------------- */
const searchInputAny = document.querySelectorAll(".search-input");
const searchIconsAny = document.querySelectorAll(".search-icon");

if (searchInputAny.length && searchIconsAny.length) {
    searchInputAny.forEach(function (searchInput, index) {
        searchInput.addEventListener("focus", function () {
            if (searchIconsAny[index]) searchIconsAny[index].classList.add("rotate-icon");
        });

        searchInput.addEventListener("blur", function () {
            if (searchIconsAny[index]) searchIconsAny[index].classList.remove("rotate-icon");
        });
    });
}

/** ---------------- Arrow rotation for collapsible items (YOUR CODE, SAFE) ----------------
 * NOTE: your HTML uses id="listopennavbar" and id="arrowicon" multiple times (invalid HTML).
 * Your selector here expects class ".listopennavbar" and ".arrowicon".
 * Keeping your selector but making it safe so it won't crash.
 */
var listItems = document.querySelectorAll("#aside .listopennavbar");
if (listItems.length) {
    listItems.forEach(function (listItem) {
...
/** ---------------- Sidebar logic (FIXED + MOBILE OPEN/CLOSE) ---------------- */
document.addEventListener("DOMContentLoaded", function () {
    const aside = document.getElementById("aside");
    const sidebariconOnly = document.getElementById("sidebaricononlys");
    const sidenavCloseBtn = document.querySelector("[sidenav-close]");

    if (sidebariconOnly && aside) {
        sidebariconOnly.onclick = function myFunction() {
            // ✅ MOBILE/TABLET: open/close the sidebar
            // xl breakpoint ~ 1280; adjust if you want
            if (window.innerWidth < 1280) {
                // your aside starts with "-translate-x-full"
                aside.classList.toggle("translate-x-0");
                aside.classList.toggle("-translate-x-full");
                aside.classList.toggle("shadow-soft-xl");

                if (sidenavCloseBtn) {
                    sidenavCloseBtn.classList.toggle("hidden");
                }
                return;
            }

            // ✅ DESKTOP: your existing "icon-only" collapse logic
            var icons = document.getElementsByClassName("sidebar-icon");
            for (var i = 0; i < icons.length; i++) {
                var icon = icons[i];
                if (icon.style.display === "none") {
    ...
    // ✅ Close button (mobile)
    if (sidenavCloseBtn && sidebariconOnly) {
        sidenavCloseBtn.addEventListener("click", function () {
            sidebariconOnly.click();
        });
    }

    // ✅ Expand sidebar when click on white sidebar (YOUR CODE, SAFE)
    if (aside) {
        aside.onclick = function myFunction() {
            if (window.innerWidth > 1024) {
                var collapsedSidebars = document.getElementsByClassName("sidebar-icon");
...
/** ---------------- CheckAll For Checkbox in tables (SAFE) ---------------- */
const headerCheckbox = document.getElementById("checkboxAll");
const bodyCheckboxes = document.querySelectorAll('#multiselect-table tbody input[type="checkbox"]');

if (headerCheckbox && bodyCheckboxes.length) {
    headerCheckbox.addEventListener("click", () => {
        bodyCheckboxes.forEach((checkbox) => {
            checkbox.checked = headerCheckbox.checked;
        });
...
document.addEventListener("DOMContentLoaded", function () {
    const tableCheckboxes = document.querySelectorAll('#multiselect-table input[type="checkbox"]');
    if (tableCheckboxes.length) {
        tableCheckboxes.forEach(function (checkbox) {
            checkbox.addEventListener("change", function () {
                ...
                var deleteAllButton = document.getElementById("deleteAllbtn");
                if (deleteAllButton) {
                    deleteAllButton.hidden = !isChecked;
                }
            });
        });
    }
...
if (deleteAllbtn && selected_ids) {
    deleteAllbtn.onclick = function () {
        let selected = new Array();

        document
            .querySelectorAll('#multiselect-table input[type="checkbox"]:checked')
